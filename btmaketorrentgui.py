...
class DownloadInfo:
    def __init__(self):
        frame = wxFrame(None, -1, 'BitTorrent Torrent File Maker',
                        size=wxSize(550, 410))
        self.frame = frame
...
#        EVT_DROP_FILES(panel, self.selectdrop)
    ...
    def selectdir(self, x):
        dl = wxDirDialog(
            self.frame, style=wxDD_DEFAULT_STYLE | wxDD_NEW_DIR_BUTTON)
        if dl.ShowModal() == wxID_OK:
            self.dirCtl.SetValue(dl.GetPath())

    def selectfile(self, x):
        dl = wxFileDialog(self.frame, 'Choose file or directory to use', '',
                          '', '', wxOPEN)
        if dl.ShowModal() == wxID_OK:
            self.dirCtl.SetValue(dl.GetPath())
    ...
    def announcecopy(self, x):
        dl = wxFileDialog(self.frame, 'Choose .torrent file to use', '', '',
                          '*.torrent', wxOPEN)
        if dl.ShowModal() == wxID_OK:
            try:
...
class CompleteDir:
    ...
    def begin(self):
        if self.separatetorrents:
            ...
            frame = wxFrame(None, -1, 'BitTorrent make torrent',
                            size=wxSize(550, 250))
        self.frame = frame
        ...
        if self.separatetorrents:
            ...
            self.currentLabel = wxStaticText(
